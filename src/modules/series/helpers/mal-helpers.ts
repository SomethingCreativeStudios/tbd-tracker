...
export async function findFromMAL(seriesName: string) {
  ...
  const newSeries = await Promise.all(anime.map((animeById) => createFromMAL(animeById)));

  return newSeries.filter((show) => show.imageUrl);
}

export async function createFromMALName(seriesName: string) {
  const foundAnime = await Search.search(seriesName, 'anime', 1);
  ...
  return createFromMAL(await Anime.byId(foundAnime.results[0].mal_id));
}

export async function createFromMALSeason(animeModel: AnimeSeason, options: { autoMatchFolders: boolean }) {
...
export async function createFromMAL(animeModel: AnimeById, options?: { autoMatchFolders: boolean }) {
  const series = new Series();
